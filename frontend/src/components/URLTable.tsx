...
import { URL, CrawlStatus } from '../types';
import { 
  Search, 
  Play, 
  Square, 
...
export const URLTable: React.FC<URLTableProps> = ({ onViewDetails }) => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['urls', page, search, sortBy, sortOrder],
    queryFn: () => urlAPI.getAll({ 
      page, 
      limit, 
      search, 
      sort_by: sortBy, 
  ...
  const handleBulkAction = (action: 'start' | 'stop' | 'delete' | 'recrawl') => {
            ...
            {urls.map((url) => {
              ...
              return (
                <tr key={url.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${statusColors[url.status]}`}>
                      {url.status}
                    </span>
                  </td>
